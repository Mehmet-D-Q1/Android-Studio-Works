...
class AdapterCard(
    ...
    override fun onBindViewHolder(holder: DataCardViewHolder, position: Int) {
        ...
        holder.itemCardDesignCardBinding.buttonDeleteInCard.setOnClickListener {
                //deleteFoodsCardWithSnackBar(foodItem.sepetYemekId,foodItem.kullaniciAdi)
                Log.e("deleteFoodssnackbarli", "deleteFoodsCardWithSnackBar tıklandı.")
                Snackbar.make(it, "Do you want to delete the selected product?", Snackbar.LENGTH_SHORT)
                    .setAction("Yes") {

                        Log.e("deleteFoodsCard", "deleteFoodsCard tıklandı.")
    ...
    override fun getItemCount(): Int {
        Log.e("item", "${ foodsListInBasket.count() }")
